...
const permissions = [
  ...
  ['view_voice_notes', 'عرض الملاحظات الصوتية'],
  ['create_voice_notes', 'تسجيل وتفريغ الملاحظات الصوتية'],
...
function MemberEditor({
  ...
  const save = async () => {
    ...
    try {
      await apiClient.patch(
        `/children/${childId}/care-team/members/${member.membership_id}`,
        {
  ...
  if (member.is_primary_guardian) {
    return <span className="primary-owner-label">التحكم الرئيسي</span>
  }
  ...
  return (
    <div className="member-admin-actions">
      ...
      <button className="text-action danger" disabled={busy} onClick={revoke}>
        إلغاء الوصول
      </button>
...
export default function CareTeamPage() {
  ...
  const [selectedPermissions, setSelectedPermissions] =
    useState<string[]>(defaultProviderPermissions)
  const [submitting, setSubmitting] = useState(false)

  const canManageTeam = useMemo(
    () =>
      child?.access_role === 'guardian' &&
      (child.guardian_type === 'primary' ||
        child.access_permissions.includes('manage_care_team')),
    [child],
  )
  const canManagePermissions = useMemo(
    () =>
      child?.access_role === 'guardian' &&
      (child.guardian_type === 'primary' ||
        child.access_permissions.includes('manage_permissions')),
    [child],
  )
  ...
  useEffect(() => {
    setSelectedPermissions(
      targetRole === 'guardian'
        ? defaultGuardianPermissions
        : defaultProviderPermissions,
    )
  }, [targetRole])
    ...
    try {
      ...
      setError(
        typeof detail === 'string'
          ? detail
          : 'تعذر إرسال الدعوة. تأكدي من البريد والصلاحيات.',
      )
    } finally {
      setSubmitting(false)
    }
  ...
  if (loading) {
    return (
      <div className="loading-row">
        <div className="spinner" /> جاري تحميل فريق الرعاية...
      </div>
    )
  }
  ...
  return (
    <section className="care-team-page">
      <div className="care-team-heading">
        <div>
          <span className="soft-kicker">فريق واحد · وصول واضح</span>
          <h1>فريق رعاية {child.preferred_name || child.first_name}</h1>
          <p>
            ولي الأمر يحدد من يرى ماذا، ومدة الوصول، ويمكنه إلغاء الصلاحية في
            أي وقت.
          </p>
        </div>
      ...
      <div className="care-team-summary-grid">
        ...
        <article>
          ...
          <strong>{overview.pending_invitations.length}</strong>
          <small>لم يتم قبولها بعد</small>
        </article>
        <article>
          <span>التحكم</span>
          <strong>{canManageTeam ? 'بيد ولي الأمر' : 'حسب صلاحيتك'}</strong>
          <small>كل وصول موثق وقابل للإلغاء</small>
        </article>
      </div>
        ...
        <form className="invite-panel" onSubmit={submitInvitation}>
          ...
          <div className="invite-fields">
            ...
            <label className="field-label">
              ...
              >
                <option value="">مستمر حتى الإلغاء</option>
                <option value="30">30 يومًا</option>
                <option value="90">90 يومًا</option>
                <option value="180">180 يومًا</option>
                <option value="365">سنة</option>
          ...
          <div className="permission-section">
            <strong>حددي الصلاحيات قبل إرسال الدعوة</strong>
            <p>يمكن تعديلها أو إلغاؤها لاحقًا.</p>
      ...
      <div className="team-section">
        <div className="section-heading">
          <div>
            <span className="soft-kicker">الفريق الحالي</span>
            <h2>من لديه وصول إلى الملف؟</h2>
          </div>
        </div>
          ...
          {overview.members.map((member) => (
            <article
              className={`team-member-card ${
                member.access_status === 'revoked' ? 'revoked' : ''
              }`}
              ...
              <div className="team-member-copy">
                <div className="member-title-row">
                  ...
                  {member.verification_status === 'verified' ? (
                    <span className="status-pill success">موثّق</span>
                  ) : (
                    <span className="status-pill warning">غير موثّق</span>
                  )}
                </div>
                <p>{member.role_label || 'عضو فريق الرعاية'}</p>
      ...
      {canManageTeam && overview.pending_invitations.length > 0 && (
        <div className="team-section pending-section">
          <div className="section-heading">
            <div>
              <span className="soft-kicker">بانتظار الموافقة</span>
              <h2>الدعوات المعلقة</h2>
            </div>
          </div>
          <div className="pending-invite-grid">
            {overview.pending_invitations.map((invitation) => (
